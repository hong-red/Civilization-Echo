...
// 兼容性 fetch 处理：优先使用原生 fetch (Node 18+), 否则使用 node-fetch
...
app.post("/api/kids/story", async (req, res) => {
  ...
  try {
    if (!KIMI_API_KEY) throw new Error("缺少KIMI_API_KEY环境变量");
...
app.post("/api/sushi", async (req, res) => {
  ...
  const fetch = await getFetch();
  if (!question) return res.status(400).json({ answer: "缺少问题参数" });
...
app.post("/api/creation/continue", async (req, res) => {
  ...
  const fetch = await getFetch();
  if (!firstLine) return res.status(400).json({ continuation: "请先写下你的开头哦～" });
...
// 关键修复：在 Vercel 环境下不调用 app.listen
